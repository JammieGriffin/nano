  ...
  <div class="max-w-[720px]">
    <ul class="m-0 p-0">
      <li v-for="page in blogList" :key="page.url" class="pt-6 pb-4">
        ...
        >
          <div
            :class="{ 'flex items-center': page.frontmatter.tags?.length > 0 }"
          >
...
const blogListRaw = computed(() => {
  const query = activeLink.value?.split('?') ?? ['','']
  const filterTag = new URLSearchParams(query[1]).get('tag')

  return blogData.filter((page) =>
    filterTag
      ? page.frontmatter.tags && page.frontmatter.tags.includes(filterTag)
      : true
  )
})
